...
Собраны из этого репозитория: `pio run -e mks_dlc32_v2_1_tmc` (+`-t buildfs` для
...
> ⚠️ **Это сборка варианта `mks_dlc32_v2_1_tmc`** — драйверы TMC2209 настраиваются
> по UART, экран не используется, coolant M7/M8 недоступен (его пины заняты шиной).
> Шине нужна навесная обвязка согласования уровней: логика драйверов на плате
> пятивольтовая, ESP32 — 3.3 В. Схема, номиналы и обоснование —
> [`docs/TMC2209_UART.md`](../docs/TMC2209_UART.md). **Без обвязки UART не заработает,
> а RX, заведённый напрямую, способен повредить GPIO ESP32.** Сам станок при этом
> ездит как обычно: step/dir идут через I2S и от UART не зависят.
> На живом станке эта сборка ещё не проверялась.
