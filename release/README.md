...
Настройки платы (**Wi-Fi, `$$`-параметры GRBL**) при прошивке скриптами
`flash_usb.*` **сохраняются** — они лежат в NVS-разделе (`0x9000`), который
скрипты не трогают.

### Единый образ для чистой установки

`MKS_DLC32_full_8MB.bin` (если приложен) — все разделы в одном файле, прошивается
на адрес `0x0`:

```bash
esptool.py --chip esp32 --port /dev/ttyUSB0 --baud 921600 \
  write_flash --flash_mode dout --flash_freq 80m --flash_size 8MB 0x0 MKS_DLC32_full_8MB.bin
```

Удобен для веб-флешеров (ESP Web Tools, MKS Flash Tool). **Внимание:** этот образ
перезаписывает NVS → **настройки Wi-Fi сбрасываются** (плата поднимет точку доступа
`MKS_DLC`). Чтобы сохранить настройки — прошивайте раздельными bin через `flash_usb.*`.
