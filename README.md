...
- ✅ SD-печать честно сообщает о сбое: слишком длинная строка (от 255 символов) или ошибка
  чтения больше не выдаются за успешное завершение (`SD Print Finish!` посреди файла).
  Заодно устранена запись за границу буфера на строке ровно в 255 символов.
- ✅ Инициализация языка экрана: присваивания вместо сравнений (`=` в `mc_language_init`)
  убраны, английская таблица заполняется явно.
- ✅ Read-only команды (`$$`, `$+`, `$L`, `$S`, `$SC`, `$CMD`) переведены на явный `anyState`
  вместо сломанного условия `state == Cycle && state == Hold`.
- ✅ `$Axis/Current/Run=0` у Trinamic-драйверов больше не программирует максимальный ток
  (`rms_current(0)` давал `CS = -1` → 255 → обрезка до 31, ~1.77 А RMS при Rsense 0.11);
  микрошаг принимается только из ряда 0/2/4…256.
