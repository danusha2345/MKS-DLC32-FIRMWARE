...
- ✅ Wi-Fi: авто-реконнект STA при обрыве, mDNS в AP и STA (`http://<hostname>.local`).
- ✅ SD ↔ USB: во время печати с карты по USB/сети проходят realtime и read-only
  запросы состояния (для хост-мониторинга), движение остаётся заблокированным.
- ✅ WebUI: оживлён канал WebUI↔ядро (вкл. `ENABLE_SERIAL2SOCKET_IN/OUT`) — вкладка
  настроек больше не висит (#25); имя устройства по умолчанию `mks-dlc32`, меняется в WebUI.
- 🔜 Проверить на железе end-to-end: WebUI (настройки/jog/OTA-заливка), Telnet, SD↔USB.
